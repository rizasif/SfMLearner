...
class DataLoader(object):
    ...
    def load_train_batch(self):
        ...
        imagepaths = tf.convert_to_tensor(file_list, dtype=tf.string)
        image, label = tf.train.slice_input_producer([imagepaths, file_list], shuffle=True)
        # Read images from disk
        image_seq = tf.read_file(image)
        image_seq = tf.image.decode_jpeg(image_seq, channels=3)
        # Resize images to a common size
        # image_seq = tf.image.resize_images(imaage, [self.img_height, self.img_width])
        image_seq.set_shape([self.img_height, self.img_width, 3])
        ...
        tgt_image, label = tf.train.batch([tgt_image, label], batch_size=self.batch_size)
